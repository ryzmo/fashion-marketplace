...
export default function NewArrivalSection() {
  ...
  useEffect(() => {
    const fetchNewProducts = async () => {
      ...
      try {
        const res = await fetch(`/api/search?category=NEW`);
    ...
    setLoadingProducts(false); // ⬅️ selesai loading
  }
    };
    ...
    <section className="md:py-16 pt-12 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-center mb-10">
          ...
          <h2 className="mx-4 text-lg font-bold px-6 py-2 bg-black text-white">
            NEW ARRIVAL
          </h2>
          <div className="flex-grow h-px bg-gray-300" />
        </div>
    ...
    : products.slice(0, 8).map((product) => (
      ...
      <div className="text-sm">
        {product.discountPrice ? (
          <p>
            <span className="line-through text-gray-500 mr-1 text-xs">
